"""
Simply FCN segmentation model train on KITTI road dataset


"""
import os.path
import tensorflow as tf
import util.seg_fcn_utils as helper
import warnings
from distutils.version import LooseVersion
import csv
import time

model_path = './checkpoints/model_fcn_road.ckpt'
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for i in range(epochs):
        ...
        for image, label in get_batches_fn(batch_size):
            ...
            print("Epoch: {}, batch loss: = {:.3f}".format(i, loss))
            training_samples += 1
            training_loss += loss
        ...
        log_writer.writerow(
            {'learning_rate': lr, 'exec_time (s)': round(training_time, 2), 'training_loss': round(training_loss, 4)})
        log_file.flush()
...
def run():
    num_classes = 2
    image_shape = (160, 576)
    data_dir = '/media/jintain/sg/permanent/datasets/KITTI'
    runs_dir = './log'
...
def predict_images(test_data_path, print_speed=False):
    ...
    # Path to vgg model
    vgg_path = os.path.join('./checkpoints', 'vgg')
...
if __name__ == '__main__':
    ...
    if training_flag:
        ...
        test_data_path = '/media/jintain/sg/permanent/datasets/KITTI/videos/2011_09_26/2011_09_26_drive_0106_sync/image_02/data'
